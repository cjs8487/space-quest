...
public class DescAllocator {
    ...
    public DescAllocator(PhysicalDevice physDevice, Device device) {
    ...
    private static Map<Integer, Integer> createDescLimits(PhysicalDevice physDevice) {
        ...
        descLimits.put(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                Math.min(maxDescs, limits.maxDescriptorSetSamplers()));
    ...
    public synchronized void freeDescSet(Device device, String id) {
        ...
        Arrays.asList(descSetInfo.descSets)
                .forEach(d -> descPoolInfo.descPool.freeDescriptorSet(device, d.getVkDescriptorSet()));
    }
