...
public class VertexBufferStruct {
    ...
    public VertexBufferStruct() {
        ...
        int offset = 0;
        // Position
        viAttrBuffer.get(i).binding(0).location(i).format(VK_FORMAT_R32G32B32_SFLOAT).offset(offset);

        // Texture coordinates
        i++;
        offset += POSITION_COMPONENTS * VulkanUtils.FLOAT_SIZE;
        viAttrBuffer.get(i).binding(0).location(i).format(VK_FORMAT_R32G32_SFLOAT).offset(offset);

        int stride = offset + TEX_COORD_COMPONENTS * VulkanUtils.FLOAT_SIZE;
        viBindings.get(0).binding(0).stride(stride).inputRate(VK_VERTEX_INPUT_RATE_VERTEX);

        vi.sType$Default().pVertexBindingDescriptions(viBindings).pVertexAttributeDescriptions(viAttrBuffer);
    }
