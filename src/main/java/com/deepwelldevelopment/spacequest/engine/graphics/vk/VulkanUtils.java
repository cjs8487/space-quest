...
public class VulkanUtils {
    ...
    public static void imageBarrier(MemoryStack stack, VkCommandBuffer cmdHandle, long image, int oldLayout,
            int newLayout, long srcStage, long dstStage, long srcAccess, long dstAccess, int aspectMask) {
        var barrier = VkImageMemoryBarrier2.calloc(1).sType$Default().oldLayout(oldLayout).newLayout(newLayout)
                .srcStageMask(srcStage).dstStageMask(dstStage).srcAccessMask(srcAccess).dstAccessMask(dstAccess)
                .srcQueueFamilyIndex(VK_QUEUE_FAMILY_IGNORED).dstQueueFamilyIndex(VK_QUEUE_FAMILY_IGNORED)
                .subresourceRange(r -> r.aspectMask(aspectMask).baseMipLevel(0).levelCount(VK_REMAINING_MIP_LEVELS)
                        .baseArrayLayer(0).layerCount(VK_REMAINING_ARRAY_LAYERS))
                .image(image);

        VkDependencyInfo dependencyInfo = VkDependencyInfo.calloc(stack).sType$Default().pImageMemoryBarriers(barrier);
    ...
    public static VulkanBuffer createHostVisibleBuff(VulkanContext vkCtx, long buffSize, int usage, String id,
        ...
        var buff = new VulkanBuffer(vkCtx, buffSize, usage, VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE,
    ...
    public static VulkanBuffer[] createHostVisibleBuffs(VulkanContext vkCtx, long buffSize, int numBuffs, int usage,
        ...
        for (int i = 0; i < numBuffs; i++) {
            result[i] = new VulkanBuffer(vkCtx, buffSize, usage, VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE,
                    VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
