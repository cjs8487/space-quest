...
public class CommandBuffer {
    ...
    public CommandBuffer(VulkanContext context, CommandPool commandPool, boolean primary, boolean oneTimeSubmit) {
        ...
        try (var stack = MemoryStack.stackPush()) {
            var cmdBufAllocInfo = VkCommandBufferAllocateInfo.calloc(stack).sType$Default()
    ...
    public void beginRecording(InheritanceInfo inheritanceInfo) {
        try (var stack = MemoryStack.stackPush()) {
            ...
            if (!primary) {
                ...
                var renderingInfo = VkCommandBufferInheritanceRenderingInfo.calloc(stack).sType$Default()
                        .depthAttachmentFormat(inheritanceInfo.depthFormat).pColorAttachmentFormats(pColorFormats)
                        .rasterizationSamples(inheritanceInfo.sampleCount);
                var vkInheritanceInfo = VkCommandBufferInheritanceInfo.calloc(stack).sType$Default()
                        .pNext(renderingInfo);
    ...
    public void submitAndWait(VulkanContext context, Queue queue) {
        ...
        fence.reset(context);
        try (var stack = MemoryStack.stackPush()) {
            var cmds = VkCommandBufferSubmitInfo.calloc(1, stack).sType$Default().commandBuffer(vkCommandBuffer);
