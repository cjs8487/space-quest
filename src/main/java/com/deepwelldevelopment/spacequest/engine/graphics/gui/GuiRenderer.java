...
import static com.deepwelldevelopment.spacequest.engine.graphics.vk.VulkanUtils.MAX_IN_FLIGHT;
...
import com.deepwelldevelopment.spacequest.engine.graphics.post.PostRenderer;
...
import com.deepwelldevelopment.spacequest.engine.graphics.vk.CommandPool;
...
import com.deepwelldevelopment.spacequest.engine.graphics.vk.Queue;
...
public class GuiRenderer {
    ...
    private static final String GUI_FRAGMENT_SHADER_FILE_GLSL = "resources/shaders/gui.frag";
    private static final String GUI_FRAGMENT_SHADER_FILE_SPV = GUI_FRAGMENT_SHADER_FILE_GLSL + ".spv";
    private static final String GUI_VERTEX_SHADER_FILE_GLSL = "resources/shaders/gui.vert";
    ...
    public GuiRenderer(EngineContext engineContext, VulkanContext vkContext, Queue queue, Attachment dstAttachment) {
        attachmentInfoColor = createColorAttachmentInfo(dstAttachment);
        renderInfo = createRenderInfo(dstAttachment, attachmentInfoColor);

        ShaderModule[] shaderModules = createShaderModules(vkContext);
        textDescSetLayout = new DescSetLayout(vkContext, new DescSetLayout.LayoutInfo(
                VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0, 1, VK_SHADER_STAGE_FRAGMENT_BIT));

        pipeline = createPipeline(vkContext, shaderModules, new DescSetLayout[] { textDescSetLayout });
        Arrays.asList(shaderModules).forEach(s -> s.cleanup(vkContext));

        vertexBuffers = new VulkanBuffer[MAX_IN_FLIGHT];
        indexBuffers = new VulkanBuffer[MAX_IN_FLIGHT];

        fontsTexture = initUI(vkContext, queue);
        var textureSamplerInfo = new TextureSamplerInfo(VK_SAMPLER_ADDRESS_MODE_REPEAT,
                VK_BORDER_COLOR_INT_OPAQUE_BLACK, 1, true);
        fontsTextureSampler = new TextureSampler(vkContext, textureSamplerInfo);
        Device device = vkContext.getDevice();
        DescAllocator descAllocator = vkContext.getDescAllocator();
        DescSet descSet = descAllocator.addDescSets(device, DESC_ID_TEXT, 1, textDescSetLayout)[0];
        descSet.setImage(device, fontsTexture.getImageView(), fontsTextureSampler,
                textDescSetLayout.getLayoutInfo().binding());

        KeyboardInput ki = engineContext.window().getKeyboardInput();
    ...
    private static Pipeline createPipeline(VulkanContext vkCtx, ShaderModule[] shaderModules,
        ...
        var vtxBuffStruct = new GuiVertexBufferStruct();
        var buildInfo = new PipelineBuildInfo(shaderModules, vtxBuffStruct.getVi(), PostRenderer.COLOR_FORMAT)
                .setPushConstRanges(new PushConstRange[] {
                        new PushConstRange(VK_SHADER_STAGE_VERTEX_BIT, 0, VulkanUtils.VEC2_SIZE) })
    ...
    private static Texture initUI(VulkanContext vkCtx, Queue queue) {
        ImGui.createContext();
        ...
        var cmdPool = new CommandPool(vkCtx, queue.getQueueFamilyIndex(), false);
    ...
    public void loadTextures(VulkanContext vkCtx, List<GuiTexture> guiTextures, TextureCache textureCache) {
        if (guiTextures == null) {
            return;
        }
        ...
        for (int i = 0; i < numTextures; i++) {
            ...
            descSet.setImage(device, texture.getImageView(), fontsTextureSampler,
                    textDescSetLayout.getLayoutInfo().binding());
            texturesMap.put(guiTexture.id(), descSet.getVkDescriptorSet());
        }
    }

    public void render(VulkanContext vulkanContext, CommandBuffer cmdBuffer, int currentFrame,
            Attachment dstAttachment) {
        try (var stack = MemoryStack.stackPush()) {
            updateBuffers(vulkanContext, currentFrame);
            if (vertexBuffers[currentFrame] == null) {
                return;
            }
            ...
            vtxBuffer.put(0, vertexBuffers[currentFrame].getBuffer());
            LongBuffer offsets = stack.mallocLong(1);
            offsets.put(0, 0L);
            vkCmdBindVertexBuffers(cmdHandle, 0, vtxBuffer, offsets);
            vkCmdBindIndexBuffer(cmdHandle, indexBuffers[currentFrame].getBuffer(), 0, VK_INDEX_TYPE_UINT16);
            ...
            for (int i = 0; i < numCmdLists; i++) {
                ...
                for (int j = 0; j < cmdBufferSize; j++) {
                    ...
                    if (textId == 0) {
                        ...
                        textDescSet = texturesMap.get(textId);
                    }
    ...
    private void updateBuffers(VulkanContext vkCtx, int idx) {
        ImDrawData imDrawData = ImGui.getDrawData();

        if (imDrawData.ptr == 0) {
            return;
        }
        int vertexBufferSize = imDrawData.getTotalVtxCount() * GuiVertexBufferStruct.VERTEX_SIZE;
        int indexBufferSize = imDrawData.getTotalIdxCount() * VulkanUtils.SHORT_LENGTH;

        if (vertexBufferSize == 0 || indexBufferSize == 0) {
            return;
        }
        var vtxBuffer = vertexBuffers[idx];
        if (vtxBuffer == null || vertexBufferSize > vtxBuffer.getRequestedSize()) {
            if (vtxBuffer != null) {
                vtxBuffer.cleanup(vkCtx);
            }
            vtxBuffer = new VulkanBuffer(vkCtx, vertexBufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                    VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE, VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
            vertexBuffers[idx] = vtxBuffer;
        }

        var indicesBuffer = indexBuffers[idx];
        if (indicesBuffer == null || indexBufferSize > indicesBuffer.getRequestedSize()) {
            if (indicesBuffer != null) {
                indicesBuffer.cleanup(vkCtx);
            }
            indicesBuffer = new VulkanBuffer(vkCtx, indexBufferSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                    VMA_MEMORY_USAGE_AUTO_PREFER_DEVICE, VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT,
                    VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
            indexBuffers[idx] = indicesBuffer;
        }
    ...
    public void resize(VulkanContext vkCtx, Attachment dstAttachment) {
        ImGuiIO imGuiIO = ImGui.getIO();
        VkExtent2D swapChainExtent = vkCtx.getSwapChain().getSwapChainExtent();
        imGuiIO.setDisplaySize(swapChainExtent.width(), swapChainExtent.height());
    ...
    public void cleanup(VulkanContext vkCtx) {
        ...
        Arrays.stream(vertexBuffers).filter(Objects::nonNull).forEach(b -> b.cleanup(vkCtx));
        Arrays.stream(indexBuffers).filter(Objects::nonNull).forEach(b -> b.cleanup(vkCtx));
        renderInfo.free();
        attachmentInfoColor.free();
    }
