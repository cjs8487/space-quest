    ...
    public DescPool(Device device, List<DescTypeCount> descTypeCounts) {
        ...
        try (var stack = MemoryStack.stackPush()) {
            ...
            for (int i = 0; i < numTypes; i++) {
                ...
                typeCounts.get(i).type(descTypeCounts.get(i).descType()).descriptorCount(descTypeCounts.get(i).count());
            }

            var descriptorPoolInfo = VkDescriptorPoolCreateInfo.calloc(stack).sType$Default()
                    .flags(VK_DESCRIPTOR_POOL_CREATE_FREE_DESCRIPTOR_SET_BIT).pPoolSizes(typeCounts).maxSets(maxSets);
